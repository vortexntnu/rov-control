...
// All variables are named based on how they appear in the paper.
...
public:
  ...
  Eigen::Vector6d getRestoring(const Eigen::Quaterniond &q) const;
...
private:
  Eigen::Matrix6d proportionalGainMatrix(const Eigen::Matrix3d &R) const;
  Eigen::Vector6d errorVector(const Eigen::Vector3d    &p,
                              const Eigen::Vector3d    &p_d,
                              const Eigen::Quaterniond &q,
                              const Eigen::Quaterniond &q_d) const;
  Eigen::Vector6d restoringForceVector(const Eigen::Matrix3d &R) const;
  ...
  const Eigen::Vector3d m_r_G;  // Center of gravity, expressed in body frame
  const Eigen::Vector3d m_r_B;  // Center of buoyancy, expressed in body frame
  const double m_W;             // [N] Weight of ROV
  const double m_B;             // [N] Buoyancy of ROV
};
